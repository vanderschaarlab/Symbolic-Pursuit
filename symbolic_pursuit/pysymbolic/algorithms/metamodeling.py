...
def eval_one_dimension(Gmodel, X, midpoint=0.5):
    ...
    try:
        ...
        Gmodel.Taylor_poly_ = taylor(
            Gmodel.math_expr, midpoint, Gmodel.approximation_order
        )
        coeffp = Gmodel.Taylor_poly_[::-1]

        approx_expr = [
            coeffp[k] * ((X - midpoint) ** (Gmodel.approximation_order - k))
            for k in range(Gmodel.approximation_order)
        ]
...
def eval_one_dimension_(Meijer_G_func, dim_name, vals):
    ...
    evaluated = [
        simplify(
            sympify(str(re(Meijer_G_func.approx_expression()))).subs(dim_name, vals[k])
        ).evalf()
...
def eval_Kolmogorov_gradient_only(const_, X, theta, Orders, Orders_out, h):
    ...
    f_val_0, f_inners = eval_Kolmogorov_expression_only(
        const_, X, theta, Orders, Orders_out
    )

    for k in range(tot_num_params):

        f_inners_h = deepcopy(f_inners)
        inner_funcs_h, outer_funcs_h = get_inner_outer_functions(
            n_dim, theta_shifts[k, :], Orders, Orders_out
        )
        idx = int(np.floor(k / num_inner_params))

        if k < num_inner_params * n_dim:

            f_inners_h[idx, :] = eval_inner_functions_only(
                X[:, idx].reshape((-1, 1)), [inner_funcs_h[idx]]
            )

        f_vals_h.append(
            eval_Kolmogorov_expression_(const_, X, f_inners_h, outer_funcs_h)
        )

    f_grad = (
        np.concatenate(f_vals_h, axis=1) - np.tile(f_val_0, [1, tot_num_params])
    ) / h
...
def eval_Kolmogorov_expression_only(const_, X, theta, Orders, Orders_out):
    ...
    inner_funcs, outer_funcs = get_inner_outer_functions(
        n_dim, theta, Orders, Orders_out
    )
...
def stretch_G_params(G):
    ...
    stretched_params_ = np.concatenate(
        [
            np.concatenate(G.a_p).reshape((-1, 1)),
...
def MeijerG_finite_difference(G, h):
    ...
    finite_diff_params = (
        np.tile(stretched_params_, [num_params, 1]) + np.eye(num_params) * h
    )
...
def eval_inner_functions_only(X, inner_funcs):
    ...
    f_inners_ = np.real(
        np.array(
            [
                eval_one_dimension(inner_funcs[u], X[:, u])
                for u in range(len(inner_funcs))
            ]
        )
    )
...
def exact_Kolmogorov_expression(n_dim, theta, Orders, Orders_out, const_, r=1):
    ...
    Thetas, Thetas_out = get_theta_parameters(
        theta.reshape((-1,)), Orders, Orders_out, n_dim
    )
...
class Symbolic_Metamodel:
    ...
    def get_exact_Kolmogorov_expression(self, theta):
        ...
        Thetas, Thetas_out = get_theta_parameters(
            theta.reshape((-1,)), self.Orders_in, self.Orders_out, self.n_dim
        )
        Thetas_in_0, Thetas_out_0 = get_theta_parameters(
            self.theta_0.reshape((-1,)), self.Orders_in, self.Orders_out, self.n_dim
        )
        ...
        inner_funcs = [
            MeijerG(theta=Thetas[k], order=self.Orders_in[k]) for k in range(self.n_dim)
        ]
    ...
    def initialize_thetas(self):
        ...
        self.dim_combins = list(
            self.poly._combinations(
                self.single_dim, degree=2, interaction_only=True, include_bias=False
            )
        )
        ...
        if hasattr(self.pred_model, "predict_proba"):
            ...
            initializer_model.fit(
                self.x_train,
                (
                    self.pred_model.predict_proba(self.origin_x_train)[:, 1]
                    > binarized_thresh
                )
                * 1,
            )
            ...
            initializer_model.fit(
                self.x_train,
                (self.pred_model.predict(self.origin_x_train) > binarized_thresh) * 1,
            )
    ...
    def Loss_grad(self, theta):
        ...
        subsamples_ = np.random.choice(
            list(range(self.x_train.shape[0])), size=self.batch_size, replace=False
        )

        x_ = self.x_train[subsamples_, :]
        x_original = self.origin_x_train[subsamples_, :]

        f_est, f_grad = eval_Kolmogorov_gradient_only(
            self.init_scale,
            x_,
            theta.reshape((-1,)),
            self.Orders_in,
        ...
        if loss_type == "mean_square_error":
            ...
            loss_per_param = np.tile(
                -(f_true - f_est) / ((1 - f_est) * f_est), [1, f_grad.shape[1]]
            )
            loss_grad = np.mean(loss_per_param * f_grad, axis=0).reshape((-1, 1))
            loss_ = np.mean(
                -1
                * (
                    np.multiply(f_true, np.log(f_est))
                    + np.multiply((1 - f_true), np.log(1 - f_est))
                )
            )
    ...
    def evaluate(self, x_in):
        ...
        y_est, _ = eval_Kolmogorov_gradient_only(
            self.init_scale,
        ...
        if self.exact_grad:
            ...
            for var in vars_:
                ...
                for k in range(x_test.shape[0]):
                    ...
                    grad_.append(
                        [
                            gradients_[v]
                            .subs(vars_[0], x_test[k, 0])
                            .subs(vars_[1], x_test[k, 1])
                            .subs(vars_[2], x_test[k, 2])
                            .subs(vars_[3], x_test[k, 3])
                            .subs(vars_[4], x_test[k, 4])
                            .subs(vars_[5], x_test[k, 5])
                            .subs(vars_[6], x_test[k, 6])
                            .subs(vars_[7], x_test[k, 7])
                            .subs(vars_[8], x_test[k, 8])
                            .subs(vars_[9], x_test[k, 9])
            ...
            for u in range(x_in.shape[1]):
                ...
                if self.feature_types is not None:
                    ...
                    x_in_h[0, u] = ((self.feature_types[u] == "c") * 1) * (
                        x_in_h[0, u] + h
                    ) + ((self.feature_types[u] == "b") * 1) * ((x_in_h[0, u] == 0) * 1)
                ...
                grad_.append(
                    np.abs(
                        ...
                        - self.exact_pred_expr.predict_proba(x_in)[:, 1]
                    )
                    / h
                )
