...
from keras.layers import Add, Dense, Flatten, Input, Lambda, Multiply
...
def Symbolic_reg_expr(X, y):
    ...
    est_gp = SymbolicClassifier(parsimony_coefficient=0.01, random_state=1)
...
def get_symbolic_reg_scores(model, x_train, x_test):
    ...
    for k in range(x_test.shape[0]):
        ...
        scores_.append(
            [
                gradients_[v].subs(tuple(vars_), tuple(list(x_test[k, :])))
...
def get_lime_scores(predictive_model, x_train, x_test):
    ...
    for w in range(x_test.shape[0]):
        exp = explainer.explain_instance(
            x_test[w], predictive_model.predict_proba, num_features=10
        )
        ...
        curr_scores = [
            np.where(
                np.array(
                    [
                        pd.Series(rank_list[v][0]).str.contains("X" + str(k))[0] * 1
                        for k in range(10)
                    ]
                )
                == 1
...
def get_shap_scores(predictive_model, x_train, x_test):
    ...
    explainer = shap.KernelExplainer(
        predictive_model.predict_proba, x_train, link="logit"
    )
...
def L2X(
    ...
    if train:
        ...
        model.compile(loss=loss_, optimizer=optimizer_, metrics=["acc"])
        filepath = "L2X.hdf5"
    ...
    pred_model.compile(loss=None, optimizer="rmsprop", metrics=None)  # metrics=[None])

    scores = pred_model.predict(x_val, verbose=0, batch_size=BATCH_SIZE)

    median_ranks = compute_median_rank(
        scores, k=num_selected_features, datatype_val=datatype_val
    )

    return median_ranks


def get_instancewise_median_ranks(
    dataset_, num_samples, num_selected_features, method_
):
    ...
    if method_ != "L2X":
        ...
        x_train, y_train, x_test, y_test, datatypes_val = create_data(
            dataset_, n=num_samples
        )

        eval_scores = eval_method(x_train, y_train, x_test, model_type=model_type)
        eval_ranks = compute_median_rank(
            eval_scores, k=num_selected_features, datatype_val=datatypes_val
        )
...
def Metamodel_instancewise(x_train, y_train, x_test, model_type):
    ...
    metamodel.fit(num_iter=10, batch_size=x_train.shape[0], learning_rate=0.01)

    metamodel_scores = np.array(metamodel.get_instancewise_scores(x_test)).reshape(
        (-1, x_train.shape[1])
    )
