...
def load_hyperparameter_config():
    ...
    hyperparameter_space = {
        "hyper_1": (
            ...
            [1, 0, 0, 2],
        ),  # Parent of sin , cos, sh, ch
        "hyper_2": (
            np.array([2.0, 2.0, 2.0, 1.0, 1.0]),
            [0, 1, 3, 1],
        ),  # Parent of monomials
        "hyper_3": (
            np.array([0.0, 0.1, 0.1, 0.0, 0.1, 1.0]),
            [2, 1, 2, 3],
        ),  # Parent of exp, Gamma, gamma
        "hyper_4": (
            np.array([0.0, 0.0, 1.0, 0.0, 1.0]),
            [1, 1, 2, 2],
        ),  # Parent of Step functions
        "hyper_5": (
            np.array([1.0, 1.2, 3.0, 3.3, 0.4, 1.5, 1.0]),
            [2, 2, 3, 3],
        ),  # Parent of ln, arcsin, arctg
        "hyper_6": (
            np.array([1.1, 1.2, 1.3, 1.4, 1.0]),
            [2, 0, 1, 3],
        ),  # Parent of Bessel functions
    }
...
def get_symbolic_model(f, npoints, xrange):
    ...
    for k in range(len(hyperparameter_space)):
        ...
        symbolic_model, Loss_ = symbolic_modeling(
            f,
            hyperparameter_space["hyper_" + str(k + 1)][1],
            hyperparameter_space["hyper_" + str(k + 1)][0],
            npoints,
            xrange,
        )
...
def symbolic_regressor(f, npoints, xrange):
    ...
    Y_est = np.array(
        [sympify(str(sym_reg)).subs(x, X[k]) for k in range(len(X))]
    ).reshape((-1, 1))
