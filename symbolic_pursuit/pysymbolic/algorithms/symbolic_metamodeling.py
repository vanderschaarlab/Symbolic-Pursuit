...
class symbolic_metamodel:
    def __init__(self, model, X, mode="classification"):
        ...
        self.feature_expander = PolynomialFeatures(
            2, include_bias=False, interaction_only=True
        )
        self.X = X
    ...
    def set_equation(self, reset_init_model=False):
        ...
        if reset_init_model:
            ...
            self.init_model = Ridge(
                alpha=0.1, fit_intercept=False, normalize=True
            )  # LinearRegression
    ...
    def get_gradients(self, Y_true, Y_metamodel, batch_index=None):
        ...
        for k in range(self.params.shape[0]):
            ...
            param_grads[k, :] = np.array(
                self.loss_grads(Y_true, Y_metamodel, grads_vals)
            )
    ...
    def fit(self, num_iter=10, batch_size=100, learning_rate=0.01):
        ...
        for u in self.tqdm_mode(range(self.X.shape[1])):
            ...
            self.params[u, :] = tune_single_dim(
                lr=0.1, n_iter=500, x=self.X_new[:, u], y=self.Y_r
            )
        ...
        for _ in self.tqdm_mode(range(num_iter)):
            ...
            batch_index = np.random.choice(
                list(range(self.X_new.shape[0])), size=batch_size
            )
            ...
            param_grads = self.get_gradients(
                self.Y_r[batch_index], curr_func, batch_index
            )
            self.params = self.params - learning_rate * param_grads

            coef_grads = [
                self.loss_grad_coeff(
                    self.Y_r[batch_index], curr_func, self.X_init[batch_index, k]
                )
                for k in range(self.X_init.shape[1])
            ]
            self.init_model.coef_ = self.init_model.coef_ - learning_rate * np.array(
                coef_grads
            )
    ...
    def symbolic_expression(self):
        ...
        for v in range(self.num_basis):
            ...
            f_curr = basis_expression(
                a=float(self.params[v, 0]),
                b=float(self.params[v, 1]),
                c=float(self.params[v, 2]),
            )

            sym_exact += sympify(
                str(self.init_model.coef_[v] * re(f_curr.expression()))
            ).subs(x, dims_[v])
            sym_approx += sympify(
                str(self.init_model.coef_[v] * re(f_curr.approx_expression()))
            ).subs(x, dims_[v])
    ...
    def get_gradient_expression(self):
        ...
        gradients_ = [
            diff(self.approx_expression, diff_dims[k]) for k in range(len(diff_dims))
        ]

        diff_dims = [str(diff_dims[k]) for k in range(len(diff_dims))]
        evaluator = [
            lambdify(diff_dims, gradients_[k], modules=["math"])
    ...
    def get_instancewise_scores(self, X_in):
        ...
        gards_ = [
            self._gradient(gr_exp, diff_dims, evaluator, X_in[k, :])
